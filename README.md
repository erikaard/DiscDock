...
When MakeMKV reaches a scratched part of a DVD or Blu-ray, it can retry for hours. DiscDock offers two other ways to finish the movie.

**Best effort** reads the disc directly. DiscDock finds the movie in the disc's file system and reads only the movie and the navigation data MakeMKV needs, so extras and menus cost no time. When a block cannot be read, DiscDock skips ahead to the next moment of video, as a player does, and comes back later to retry the skipped spots. It stops retrying when that no longer recovers anything, or when you choose *Skip retrying*. MakeMKV then extracts the movie from the rescued disc image. Everything that could be read is copied exactly, with no loss of quality. If a disc is so damaged that MakeMKV cannot open it at all, DiscDock reads the file system first and lets MakeMKV find the movie in that copy.
